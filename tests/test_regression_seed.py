...
class TestRegressionSeed(unittest.TestCase):
    def test_fixed_seed_regression_signature(self):
        env = MultiAgentRLRLGym(EnvConfig(width=10, height=8, n_agents=2, max_steps=10))
        env.reset(seed=123)

        reward_trace = []
        for _ in range(4):
            _, rewards, _, _, _ = env.step({"agent_0": ACTION_WAIT, "agent_1": ACTION_WAIT})
            reward_trace.append((round(rewards["agent_0"], 3), round(rewards["agent_1"], 3)))
